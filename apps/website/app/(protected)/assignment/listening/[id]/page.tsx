...
export default function ListeningAssignmentPage(props: Props) {
    ...
    useEffect(() => {
        async function load() {
            try {
                const data = await getListeningAssignment(id);
                setAssignment(data);
            } finally {
                setLoading(false);
            }
            ...
            <div className="flex flex-1 flex-col mb-20 mt-10 ml-3">
                ...
                <div className="flex-1 flex border-x border-b border-gray-300 bg-white/80 backdrop-blur-sm shadow-sm rounded-b-2xl overflow-hidden">
                    <div className="flex-1 flex overflow-hidden">
                        {/* LEFT: Instructions & Group Stimulus */}
                                ...
                                {(activeSection.question_groups ?? []).map((group) => {
                                    return (
                                        <div key={group.id} className="pb-6 border-b-2 border-gray-200 last:border-b-0 space-y-4">
                                            ...
                                            {/* Group Level Stimulus (e.g. diagrams/images for multiple questions) */}
                                            {group.stimulus && (
                                                <div className="p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                                                    <StimulusRenderer stimulus={group.stimulus} />
                                                </div>
                                            )}
                                        </div>
                                    );
                        ...
                        {/* RIGHT: Titles, Questions, Interaction */}
                                                ...
                                                {group.questions.map((q) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            {/* Question Level Stimulus (if any) */}
                                                            {q.stimulus && <StimulusRenderer stimulus={q.stimulus} />}
