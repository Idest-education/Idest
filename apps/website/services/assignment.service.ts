...
export async function getReadingAssignment(
  id: string
): Promise<ReadingAssignmentDetail> {
  const res = await http.get(`${getAssignmentBaseUrl()}/reading/assignments/${id}`);
  return unwrapResponse<ReadingAssignmentDetail>(res.data);
}
...
export async function getReadingSubmissionResult(id: string): Promise<SubmissionResultV2> {
  const res = await http.get(`${getAssignmentBaseUrl()}/reading/submissions/${id}`);
  return unwrapResponse<SubmissionResultV2>(res.data);
}


//Writing
export async function getWritingAssignment(id: string): Promise<WritingAssignmentDetail> {
  const res = await http.get(`${getAssignmentBaseUrl()}/writing/assignments/${id}`);
  return unwrapResponse<WritingAssignmentDetail>(res.data);
}
...
// Speaking
export async function getSpeakingAssignment(id: string): Promise<SpeakingAssignmentDetail> {
  const res = await http.get(`${getAssignmentBaseUrl()}/speaking/assignments/${id}`);
  return unwrapResponse<SpeakingAssignmentDetail>(res.data);
}

export async function submitSpeaking(
  payload: { assignment_id: string; user_id: string },
  files: { audioOne: File; audioTwo: File; audioThree: File }
) {
  const form = new FormData();

  form.append("assignment_id", payload.assignment_id);
  form.append("user_id", payload.user_id);
  form.append("audioOne", files.audioOne);
  form.append("audioTwo", files.audioTwo);
  form.append("audioThree", files.audioThree);
...
// Listening
export async function getListeningAssignment(id: string): Promise<ListeningAssignmentDetail> {
  const res = await http.get(`${getAssignmentBaseUrl()}/listening/assignments/${id}`);
  return unwrapResponse<ListeningAssignmentDetail>(res.data);
}
...
export async function getListeningSubmissionResult(id: string): Promise<SubmissionResultV2> {
